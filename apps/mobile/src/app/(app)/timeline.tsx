...
export default function TimelineScreen() {
  ...
  return (
    <Screen title="Timeline" subtitle="A chronological, read-only view of family records.">
      ...
      {timeline.data?.items.map((item) => <Card key={`${item.kind}-${item.id}`}><Heading>{item.title}</Heading><Body>{formatDateTime(item.occurredAt, session.data?.workspace.timezone ?? "UTC")}</Body><Body muted>{item.kind.replace("_", " ")} · {item.status.replace("_", " ")}</Body>{item.description ? <Body>{item.description}</Body> : null}{item.kind !== "special_day" ? <ActionButton label="View details" secondary onPress={() => router.push({ pathname: "/records/[recordType]/[id]", params: { recordType: item.kind === "care" ? "care_entry" : item.kind, id: item.id, ...(item.localDate ? { localDate: item.localDate } : {}) } })} /> : null}</Card>)}
